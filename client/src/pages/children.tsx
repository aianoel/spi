...
import { Plus, Search, Filter, Eye, Edit, Trash2, Download, Baby } from "lucide-react";
...
export default function Children() {
  ...
  const { data, isLoading, error } = useQuery<ChildrenResponse>({
    queryKey: ["/api/children", { search: searchTerm, ageRange: ageRangeFilter, limit, offset }],
    queryFn: async () => {
  ...
  if (error) {
    return (
      <Alert variant="destructive">
        <AlertDescription>Failed to load children records. Please try again.</AlertDescription>
      </Alert>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-gray-600">Manage children information linked to students</p>
        </div>
        <Button
          onClick={() => {
          ...
          className="mt-4 sm:mt-0 bg-success text-white hover:bg-green-600"
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="md:col-span-2">
              ...
              <div className="relative">
                <Input
                  type="text"
                  placeholder="Search by child name, parent, school..."
                  value={searchTerm}
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="text-lg font-semibold text-gray-800">All Children Records</CardTitle>
        ...
        <CardContent className="p-0">
          {isLoading ? (
            <div className="p-8 text-center">
              ...
              <p className="mt-4 text-gray-600">Loading children records...</p>
            </div>
              ...
              <table className="w-full">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {data?.children.map((child) => (
                    <tr key={child.id} className="hover:bg-gray-50" data-testid={`row-child-${child.id}`}>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <div className="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center mr-4">
                            <Baby className="text-blue-600" size={16} />
                          </div>
                          <div>
                            ...
                            <div className="text-sm text-gray-500">
                              Age: {child.child_age || "N/A"} • {child.child_status || "N/A"}
                            </div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">Parent Student</div>
                        <div className="text-sm text-gray-500">STU-{child.student_id.toString().padStart(4, '0')}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{child.child_school || "N/A"}</div>
                        <div className="text-sm text-gray-500">{child.child_occupation || "N/A"}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
